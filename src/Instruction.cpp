...
Instruction::Instruction(Core * core, uint32_t code)
{
    ...
    case 0b0000011: // I-type 
    {
        ...
        switch (funct3)
        {
            ...
            {
                inst_tp_ = LB;
                exec_status_ = core->executeLB(rd_, rs1_, imm_);
                break;
            }
            case 0b001: // LH
            {
                inst_tp_ = LH;
                exec_status_ = core->executeLH(rd_, rs1_, imm_);
                break;
            }
            case 0b010: // LW
            {
                inst_tp_ = LW;
                exec_status_ = core->executeLW(rd_, rs1_, imm_);
                break;
            }
            case 0b100: // LBU
            {
                inst_tp_ = LBU;
                exec_status_ = core->executeLBU(rd_, rs1_, imm_);
                break;
            }
            case 0b101: // LHU
            {
                inst_tp_ = LHU;
                exec_status_ = core->executeLHU(rd_, rs1_, imm_);
                break;
            }
            default:
            {
    ...
    case 0b0010111: // U-type AUIPC
    {
        ...
        inst_tp_ = AUIPC;
        exec_status_ = core->executeAUIPC(rd_, imm_);
    ...
    case 0b0110111: // U-type LUI
    {
        ...
        inst_tp_ = LUI;
        exec_status_ = core->executeLUI(rd_, imm_);
    ...
    {   
        ...
        switch (funct3)
        {
            ...
            {
                inst_tp_ = ADDI;
                exec_status_ = core->executeADDI(rd_, rs1_, imm_);
                break;
            }
            case 0b010: // SLTI
            {
                inst_tp_ = SLTI;
                exec_status_ = core->executeSLTI(rd_, rs1_, imm_);
                break;
            }
            case 0b011: // SLTIU
            {
                inst_tp_ = SLTIU;
                exec_status_ = core->executeSLTIU(rd_, rs1_, imm_);
                break;
            }
            case 0b111: // ANDI
            {
                inst_tp_ = ANDI;
                exec_status_ = core->executeANDI(rd_, rs1_, imm_);
                break;
            }
            case 0b110: // ORI
            {
                inst_tp_ = ORI;
                exec_status_ = core->executeORI(rd_, rs1_, imm_);
                break;
            }
            case 0b100: // XORI
            {
                inst_tp_ = XORI;
                exec_status_ = core->executeXORI(rd_, rs1_, imm_);
                break;
            }
            case 0b001: // SLLI
            {
                ...
                {
                    inst_tp_ = SLLI;
                    exec_status_ = core->executeSLLI(rd_, rs1_, imm_);
                }
                else 
                {
            ...
            case 0b101:
            {
                ...
                {
                    inst_tp_ = SRLI;
                    exec_status_ = core->executeSRLI(rd_, rs1_, imm_);
                }
                else if (funct7 == 0b0100000) // SRAI
                {
                    inst_tp_ = SRAI;
                    exec_status_ = core->executeSRAI(rd_, rs1_, imm_);
                }
                else
                {
                    std::cout << "no match with command = " << code << "funct7 = " << funct7 << " funct3 = " << funct3 << std::endl;
    ...
    case 0b0110011: // R-type
    {
        ...
        {
            inst_tp_ = ADD;
            exec_status_ = core->executeADD(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b000 && funct7 == 0b0100000) // SUB
        {
            inst_tp_ = SUB;
            exec_status_ = core->executeSUB(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b001 && funct7 == 0b0000000) // SLL
        {
            inst_tp_ = SLL;
            exec_status_ = core->executeSLL(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b010 && funct7 == 0b0000000) // SLT
        {
            inst_tp_ = SLT;
            exec_status_ = core->executeSLT(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b011 && funct7 == 0b0000000) // SLTU
        {
            inst_tp_ = SLTU;
            exec_status_ = core->executeSLTU(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b100 && funct7 == 0b0000000) // XOR
        {
            inst_tp_ = XOR;
            exec_status_ = core->executeXOR(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b101 && funct7 == 0b0000000) // SRL
        {
            inst_tp_ = SRL;
            exec_status_ = core->executeSRL(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b101 && funct7 == 0b0100000) // SRA
        {
            inst_tp_ = SRA;
            exec_status_ = core->executeSRA(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b110 && funct7 == 0b0000000) // OR
        {
            inst_tp_ = OR;
            exec_status_ = core->executeOR(rd_, rs1_, rs2_);
        }
        else if (funct3 == 0b111 && funct7 == 0b0000000) // AND
        {
            inst_tp_ = AND;
            exec_status_ = core->executeAND(rd_, rs1_, rs2_);
        }
        else
        {
            std::cout << "no match with command = " << code << std::endl;
    ...
    case 0b1100111: // JALR
    {
        ...
        {
            inst_tp_ = JALR;
            exec_status_ = core->executeJALR(rd_, rs1_, imm_);
        }
        break;
    }
    case 0b1101111: // JAL
    {   
        ...
        exec_status_ = core->executeJAL(rd_, imm_);
        break;
    }
    ...
    {   
        ...
        switch (funct3)
        {
            ...
            {
                inst_tp_ = SB;
                exec_status_ = core->executeSB(rs1_, rs2_, imm_);
                break;
            }
            case 0b001: // SH
            {
                inst_tp_ = SH;
                exec_status_ = core->executeSH(rs1_, rs2_, imm_);
                break;
            }
            case 0b010: // SW
            {
                inst_tp_ = SW;
                exec_status_ = core->executeSW(rs1_, rs2_, imm_);
                break;
            }
            default:
            {
    ...
    case 0b1100011:
    {
        ...
        switch (funct3)
        {
            ...
            {
                inst_tp_ = BEQ;
                exec_status_ = core->executeBEQ(rs1_, rs2_, imm_);
                break;
            }
            case 0b001: // BNE
            {
                inst_tp_ = BNE;
                exec_status_ = core->executeBNE(rs1_, rs2_, imm_);
                break;
            }
            case 0b100: // BLT
            {
                inst_tp_ = BLT;
                exec_status_ = core->executeBLT(rs1_, rs2_, imm_);
                break;
            }
            case 0b101: // BGE
            {
                inst_tp_ = BGE;
                exec_status_ = core->executeBGE(rs1_, rs2_, imm_);
                break;
            }
            case 0b110: // BLTU
            {
                ...
                inst_tp_ = BLTU;
                exec_status_ = core->executeBLTU(rs1_, rs2_, imm_);
                break;
            }
            case 0b111: // BGEU
            {
                ...
                inst_tp_ = BGEU;
                exec_status_ = core->executeBGEU(rs1_, rs2_, imm_);
                break;
            }
            default:
            {
